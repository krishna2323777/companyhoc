  ...
  return <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full m-4">
        ...
        <div className="p-6">
          ...
          }} className={`px-4 py-2 rounded-md text-sm font-medium ${comparisonType === 'country' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              Compare Countries
            </button>
            <button onClick={() => {
            setComparisonType('business-type');
            setSelectedCountryForTypes('netherlands');
          }} className={`px-4 py-2 rounded-md text-sm font-medium ${comparisonType === 'business-type' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              Compare Business Types
            </button>
          </div>
          {comparisonType === 'country' ? <div className="flex flex-wrap gap-2 mb-6">
              {Object.entries(availableCountries).map(([key, name]) => <button key={key} onClick={() => selectedCountries.includes(key) ? removeCountry(key) : addCountry(key)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${selectedCountries.includes(key) ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`} disabled={!selectedCountries.includes(key) && selectedCountries.length >= 4}>
                  {name}
                  {selectedCountries.includes(key) && <XIcon className="inline-block ml-2 h-4 w-4" />}
                </button>)}
            </div> : <div className="space-y-6">
              <div className="flex flex-wrap gap-2 mb-6">
                {Object.entries(availableCountries).map(([key, name]) => <button key={key} onClick={() => setSelectedCountryForTypes(key)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${selectedCountryForTypes === key ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                    {name}
                  </button>)}
              </div>
              <div className="border border-gray-200 rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {Object.entries(businessTypeData[selectedCountryForTypes].types).map(([type, data]) => <button key={type} onClick={() => toggleBusinessType(type)} className={`border ${selectedBusinessTypes.includes(type) ? 'border-blue-600 bg-blue-50' : 'border-gray-200'} rounded-lg p-4 text-left hover:bg-gray-50 transition-colors ${!selectedBusinessTypes.includes(type) && selectedBusinessTypes.length >= 3 ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!selectedBusinessTypes.includes(type) && selectedBusinessTypes.length >= 3}>
              ...
              {selectedBusinessTypes.length > 0 && <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {selectedBusinessTypes.map(type => <div key={type} className="border border-gray-200 rounded-lg p-4">
            ...
            <div className="flex items-start space-x-6">
              <div className="flex-1">
                ...
                <div className="flex space-x-4">
                  <button className="bg-white text-blue-600 px-6 py-2 rounded-md text-sm font-medium hover:bg-blue-50">
                    Schedule Consultation
                  </button>
                  <button className="border border-white text-white px-6 py-2 rounded-md text-sm font-medium hover:bg-blue-700">
